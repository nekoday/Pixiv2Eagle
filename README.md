...
- 📁 自动创建画师专属文件夹
- 📂 可切换直接保存到 Pixiv 文件夹，不创建画师子文件夹
- 🏷️ 保留作品标签和元数据
- 📄 支持多页作品保存
- 🗂️ 可选为多页作品创建子文件夹
- 📝 可选保存作品描述到 Eagle 注释
- ⏰ 可选使用作品投稿时间作为添加日期
- 🔢 可选严格排序保存（按原始列表顺序写入修改时间）
...
### 文件夹 ID 设置规则
- 如果设置了 Pixiv 文件夹 ID：
  - 默认会在指定的 Pixiv 文件夹下查找或创建画师专属文件夹
  - 如果开启 `📂 切换：直接保存到 Pixiv 文件夹（不创建画师子文件夹）`，则直接保存到该文件夹
  - 如果找不到指定的 Pixiv 文件夹，会提示错误
- 如果清空文件夹 ID：
  - 默认会在 Eagle 根目录下查找或创建画师专属文件夹
  - 如果开启直接保存模式，则会直接保存到 Eagle 根目录
...
- 点击「保存到 Eagle」按钮后，脚本会自动：
  - 获取作品信息（标题、作者、标签等）
  - 检查保存基准目录（画师文件夹或 Pixiv 文件夹）
  - 下载并保存作品到 Eagle
  - 在 Eagle 内保留作品信息

### 保存目录模式
- 可以通过 Tampermonkey 菜单 `📂 切换：直接保存到 Pixiv 文件夹（不创建画师子文件夹）` 在两种模式间切换。
- 默认模式：
  - 按当前逻辑创建画师专属文件夹，再将作品保存到对应画师目录下
- 直接保存模式：
  - 不再创建画师子文件夹，作品直接保存到配置的 Pixiv 文件夹
  - 如果未设置 Pixiv 文件夹 ID，则直接保存到 Eagle 根目录
  - 页面按钮会显示为「打开保存目录」
...
### 作品子文件夹
- 对于漫画作品（`illustType = 1`）或属于 Pixiv 系列的作品，脚本会先在当前保存基准目录下创建/定位对应的系列文件夹，并始终为作品建立一个以作品标题命名的子文件夹；子文件夹的描述会写入作品 ID，系列文件夹描述会写入 Pixiv 系列 URL，方便在 Eagle 中反查来源。
- 其它作品可以通过 Tampermonkey 菜单中的 `🗂️ 切换：为多页作品创建子文件夹` 在以下模式之间循环：**关闭 → 多页 → 始终**。
  - 关闭：所有插画会直接保存到当前目录/系列文件夹中，不额外创建子文件夹
...
- 检测流程：获取当前作品信息 → 定位保存基准目录；如果作品属于系列，则进入对应系列文件夹，否则停留在当前目录。
...
### 严格排序保存
- 在 Tampermonkey 菜单中可以开启/关闭 `🔢 切换：按照严格排序保存`
- 启用该功能后：
  - 同一作品的第 0 张图片修改时间不变，后续图片在此基础上依次 +1（毫秒）
  - 基准时间优先使用「投稿时间」开关的设置；若未开启，则使用实际保存时刻
  - 适合希望在 Eagle 中按 Pixiv 原始列表顺序查看作品的用户
